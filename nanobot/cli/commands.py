...
def gateway(
    ...
    agent = AgentLoop(
        ...
        max_retries=config.agents.defaults.max_retries,
        retry_backoff_seconds=config.agents.defaults.retry_backoff_seconds,
    )
...
def logs(
    ...
    """Tail the nanobot-gateway systemd service logs."""
    import subprocess

    cmd = ["journalctl", "--user", "-u", "nanobot-gateway", f"-n{lines}"]
    if follow:
        cmd.append("-f")
    try:
        subprocess.run(cmd)
